...
class Employee(Base):
    ...
    hra = Column(Float, default=0.0)
    lta = Column(Float, default=0.0)
    other_allowance = Column(Float, default=0.0, name="other_allowance(variable pay)")
    total_addition = Column(Float, default=0.0)
...
def number_to_words(number):
    ...
    # if decimal_part > 0:
    #     words += f" and {decimal_part}/100"
    return words.capitalize() + " Rupees"
...
def calculate_net_salary(total_addition, month_year, days_worked, total_deductions):
    try:
        # Convert 'MMM-YYYY' to month and year
        month_str, year = month_year.split('-')
...
def distribute_salary(total_salary):
    basic_pay = total_salary * 0.5
    hra = total_salary * 0.25
    lta = total_salary * 0.05
    other_allowance = total_salary * 0.2
    return basic_pay, hra, lta, other_allowance

def main():
    st.sidebar.title("Navigation")
    page = st.sidebar.radio("Go to", ["Payroll Management", "Employee List"])

    if page == "Payroll Management":
...
def payroll_management_page():
    ...
    # month_year = st.text_input("Month-Year (MM-YYYY)", value=st.session_state.employee_data.get('month_year', ''))
    month_year = st.text_input("Month-Year (MMM-YYYY)", value=st.session_state.employee_data.get('month_year', ''))
    ...
    if salary_input_method == "Component-wise":
        ...
        lta = st.number_input("LTA", min_value=0.0, step=0.01, value=st.session_state.employee_data.get('lta', 0.0))
        # other_allowance = st.number_input("Other Allowance", min_value=0.0, step=0.01, value=st.session_state.employee_data.get('other_allowance', 0.0))
        # Update this line in your form input section
        other_allowance = st.number_input("Other Allowance (Variable Pay)", min_value=0.0, step=0.01, value=st.session_state.employee_data.get('other_allowance(variable pay)', 0.0))
    ...
    st.write(f"LTA: {lta:.2f}")
    st.write(f"Other Allowance: {other_allowance:.2f}")
    # st.write(f"Other Allowance (Variable Pay): {variable_pay:.2f}")
    ...
    if month_year:
        try:
            ...
            st.error("Invalid Month-Year format. Please use MMM-YYYY.")
            days_worked = st.number_input("Days Worked", min_value=0, max_value=31, step=1, value=st.session_state.employee_data.get('days_worked', 0))
    else:
        days_worked = st.number_input("Days Worked", min_value=0, max_value=31, step=1, value=st.session_state.employee_data.get('days_worked', 0))
        ...
        if not month_year or not employee_id or not employee_name:
            ...
            try:
                ...
                if existing_employee:
                    ...
                    existing_employee.lta = lta
                    existing_employee.other_allowance = other_allowance
                    # existing_employee.variable_pay = variable_pay
                    existing_employee.total_addition = total_addition
                    existing_employee.provident_fund = provident_fund
                    existing_employee.esi = esi
                    existing_employee.professional_tax = professional_tax
...
def employee_list_page():
    ...
    if not employees:
        ...
        for employee in employees:
            with st.expander(f"{employee.employee_name} - {employee.employee_id}"):
                st.write(f"Designation: {employee.designation}")
                st.write(f"Date of Joining: {employee.date_of_joining}")
                st.write(f"PAN Card: {employee.pan_card}")
                st.write(f"Latest Salary Month: {employee.month_year}")
                st.write(f"Net Salary: {employee.net_salary}")

                if st.button(f"Delete {employee.employee_name}", key=f"delete_{employee.id}"):
                    try:
                        session.delete(employee)
                        session.commit()
                        st.success(f"Employee {employee.employee_name} has been deleted.")
                        # st.experimental_rerun()
                    except Exception as e:
                        session.rollback()
                        st.error(f"An error occurred while deleting the employee: {str(e)}")
